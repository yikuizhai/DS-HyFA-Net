...
@DATA.register_func('SYSUCD_train_dataset')
def build_sysucd_train_dataset(C):
    configs = get_common_train_configs(C)
    configs.update(dict(
        transforms=(Choose(
            HorizontalFlip(), VerticalFlip(),
...
@DATA.register_func('SYSUCD_eval_dataset')
def build_sysucd_eval_dataset(C):
    configs = get_common_eval_configs(C)
    configs.update(dict(
        transforms=(None, Normalize(np.asarray(C['mu']), np.asarray(C['sigma'])), None),
        root=constants.IMDB_SYSUCD,
